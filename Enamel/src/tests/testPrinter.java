...
public class testPrinter {
	...
	/*
	 * //Commonly reused tests for new blocks public void checkOldBlock(Block block)
	 * throws InvalidCellException { if(first) first = false; else
	 * assertEquals("/~NEXTT", reader.nextLine()); assertEquals("/~disp-clearAll",
	 * reader.nextLine()); assertEquals("/~disp-cell-pins:0 " +
	 * interpreter.getPins(block.letter), reader.nextLine());
	 ...
	 * assertEquals("/~user-input", reader.nextLine()); assertEquals("/~ONEE",
	 * reader.nextLine()); assertEquals((block.answer == 1) ? "/~sound:correct.wav"
	 * : "/~sound:wrong.wav", reader.nextLine()); assertEquals((block.answer == 1) ?
	 * block.correctResponse : block.wrongResponse, reader.nextLine());
	 * assertEquals("/~skip:NEXTT", reader.nextLine()); assertEquals("/~TWOO",
	 * reader.nextLine()); assertEquals((block.answer == 2) ? "/~sound:correct.wav"
	 * : "/~sound:wrong.wav", reader.nextLine()); assertEquals((block.answer == 2) ?
	 * block.correctResponse : block.wrongResponse, reader.nextLine());
	 * assertEquals("/~skip:NEXTT", reader.nextLine()); assertEquals("",
	 * reader.nextLine()); }
	 * 
	 * public void checkOldBlock(ArrayList<Block> blocks, Block block) throws
	 * InvalidCellException { if(first) first = false; else assertEquals("/~NEXTT",
	 * reader.nextLine()); assertEquals("/~disp-clearAll", reader.nextLine());
	 * assertEquals("/~disp-cell-pins:0 " + interpreter.getPins(block.letter),
	 * reader.nextLine()); assertEquals(block.story, reader.nextLine());
	 * assertEquals("/~skip-button:0 ONEE", reader.nextLine());
	 * assertEquals("/~skip-button:1 TWOO", reader.nextLine());
	 * assertEquals("/~user-input", reader.nextLine()); assertEquals("/~ONEE",
	 * reader.nextLine()); assertEquals((block.answer == 1) ? "/~sound:correct.wav"
	 ...
	 * : "/~sound:wrong.wav", reader.nextLine()); assertEquals((block.answer == 2) ?
	 * block.correctResponse : block.wrongResponse, reader.nextLine());
	 * assertEquals("/~skip:" + blocks.get(blocks.indexOf(block) + 1),
	 * reader.nextLine()); assertEquals("", reader.nextLine()); }
	 * 
	 * 
	 * private void checkNewBlock(Block block) throws InvalidCellException {
	 * if(first) first = false; else assertEquals("/~" + block.name.toUpperCase(),
	 * reader.nextLine()); assertEquals("/~disp-clearAll", reader.nextLine());
	 * assertEquals("/~disp-string:" + block.cells, reader.nextLine());
	 * assertEquals(block.story, reader.nextLine()); for(int i = 1; i <=
	 * block.buttonsUsed; i++) {t assertEquals("/~skip-button:" + i + " " + "JUMPP"
	 * + i , reader.nextLine()); } assertEquals("/~user-input", reader.nextLine());
	 * for(int i = 1; i <= block.buttonsUsed; i++) { assertEquals("/~" + "JUMPP" +
	 * i, reader.nextLine()); assertEquals((block.answer == i) ?
	 * "/~sound:correct.wav" : "/~sound:wrong.wav", reader.nextLine());
	 * assertEquals((block.answer == i) ? block.correctResponse :
	 * block.wrongResponse, reader.nextLine()); assertEquals("/~skip:" +
	 * blocks.get(index + 1).name.toUpperCase(), reader.nextLine()); }
	 * assertEquals("", reader.nextLine()); }
	 */
	...
	private void checkNewBlock(ArrayList<Block> blocks, Block block) throws InvalidCellException {
		...
		assertEquals("/~" + block.name, reader.nextLine());
		...
		for (int i = 1; i <= block.buttonsUsed; i++) {
			assertEquals("/~skip-button:" + (i - 1) + " " + "JUMPP" + i, reader.nextLine());
		}
		...
		for (int i = 1; i <= block.buttonsUsed; i++) {
			...
			if (blocks.indexOf(block) < blocks.size() - 1)
				assertEquals("/~skip:" + blocks.get(blocks.indexOf(block) + 1).name, reader.nextLine());
			else
				assertEquals("/~skip:ENDD", reader.nextLine());
		}
		assertEquals("", reader.nextLine());
		if (blocks.indexOf(block) == blocks.size() - 1)
			assertEquals("/~ENDD", reader.nextLine());

	}

	// Tests for file creation/reading exceptions, also checks that the initial
	// block is printed correctly
	@Test
	...
	public void testSoundTag()
		...
		assertEquals("/~" + tBlock.name, reader.nextLine());
		assertEquals("/~disp-clearAll", reader.nextLine());
		if (repeat)
			assertEquals("/~repeat", reader.nextLine());
