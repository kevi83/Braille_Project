...
public class Loader {
	...
	public ArrayList<Block> start(File file) throws EOFException {
		String fileLine, lineAfter = "";
		try {
			fileScanner = new Scanner(file);
			nextLineCheck = new Scanner(file);
		...
		blocklist.add(new Block(currentBlock)); // last block of file
		fileScanner.close();
		nextLineCheck.close();
	...
	private void interpretLine(String line, String nextLine) {
		...
		if (line.equals("")) { // This my not be correct to it's intended use
			if (nextLine.equals("")) {
				...
				blocklist.add(new Block(currentBlock));
		 ...
		 * String[] param = line.substring(8).split("\\s"); // this may not be the
		 * appropriate format
		 * 
		 * if (nextLine.length() >= 7 && nextLine.substring(0, 7).equals("/~sound")) {
		 * if (nextLine.substring(7).equals("correct.wav")) { // another instance of "
		 * correct.wav" maybe incorrect setAnswer(buttonOrder.indexOf(param[1])); } } }
		 **/
		...
		// may need to change, this is so that the line which don't hold info on the
		// blocks,
	...
	private void addStory(String s) {
		currentBlock.story += s;
	}

	private void addCorrect(String c) {
		currentBlock.correctResponse += c;
	}

	private void addWrong(String w) {
		currentBlock.wrongResponse += w;
	}

	private void setAnswer(int a) {
		currentBlock.answer = a + 1;
	}

	private void setCell(String c) {
		currentBlock.cells = c;
	}

	private void setButtonsUsed(String b) {

		currentBlock.buttonsUsed = Integer.valueOf(b);
	}
	...
	private void blockClear() {
		...
		currentBlock.correctResponse = "";
		currentBlock.wrongResponse = "";
		currentBlock.cells = "a";
		currentBlock.buttonsUsed = 1;
	}
